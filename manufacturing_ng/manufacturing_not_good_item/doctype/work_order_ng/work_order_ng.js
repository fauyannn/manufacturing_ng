...
frappe.ui.form.on('Work Order NG', {
	...
	onload: function(frm) {
		...
		if(frm.doc.__islocal) {
			...
			erpnext.work_order_ng.set_default_warehouse(frm);
		}
	},

	refresh: function(frm) {
		erpnext.toggle_naming_series();
		erpnext.work_order_ng.set_custom_buttons(frm);
		frm.set_intro("");
		...
		if (d.workstation) {
			frappe.call({
				...
				callback: function (data) {
					...
					erpnext.work_order_ng.calculate_cost(frm.doc);
					erpnext.work_order_ng.calculate_total_cost(frm);
				}
			});
		}
	},
...
erpnext.work_order_ng = {
	set_custom_buttons: function(frm) {
		var doc = frm.doc;
		if (doc.docstatus === 1) {
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					erpnext.work_order_ng.stop_work_order(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					erpnext.work_order_ng.stop_work_order(frm, "Resumed");
				}, __("Status"));
			}
			...
			if (show_start_btn) {
				if ((flt(doc.material_transferred_for_manufacturing) < flt(doc.qty))
					...
					frm.add_custom_button(__('Create Pick List'), function() {
						erpnext.work_order_ng.create_pick_list(frm);
					});
					var start_btn = frm.add_custom_button(__('Start'), function() {
						erpnext.work_order_ng.make_se(frm, 'Material Transfer for Manufacture');
					});
			...
			if(!frm.doc.skip_transfer){
				...
				&& frm.doc.status != 'Stopped') {
					...
					if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
						...
						if (counter > 0) {
							var consumption_btn = frm.add_custom_button(__('Material Consumption'), function() {
								...
								erpnext.work_order_ng.make_consumption_se(frm, backflush_raw_materials_based_on);
							});
					...
					var finish_btn = frm.add_custom_button(__('Finish'), function() {
						erpnext.work_order_ng.make_se(frm, 'Manufacture');
					});
