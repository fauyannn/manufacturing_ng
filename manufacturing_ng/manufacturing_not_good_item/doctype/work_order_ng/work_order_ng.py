...
class WorkOrderNG(Document):
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = frappe.db.sql("""select sum(qty) from `tabWork Order NG`
	...
	def update_work_order_qty_in_so(self):
		...
		qty = frappe.db.sql(""" select sum(qty) from
			`tabWork Order NG` where sales_order = %s and docstatus = 1 and {0}
